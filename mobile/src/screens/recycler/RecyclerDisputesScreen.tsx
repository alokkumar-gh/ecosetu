/**
 * EcoSetu Recycler Disputes Screen
 * Canonical Reference: Marketplace Phase 7 - Dispute Resolution & Return Workflows (SIH 26229)
 *
 * Displays disputes involving recycler purchases with status filters, factual references,
 * and zero synthetic metrics.
 */
...
export const RecyclerDisputesScreen: React.FC = () => {
  const { t } = useI18n();
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Purchase Disputes & Claims</Text>
        <View style={{ width: 36 }} />
      </View>
        ...
        >
          ...
          {disputes.length === 0 ? (
            <View style={styles.emptyContainer}>
              <AppIcon name="shieldCheck" size={48} color="#94A3B8" />
              <Text style={styles.emptyTitle}>No Active Disputes</Text>
              <Text style={styles.emptyDesc}>
                {selectedFilter === 'ALL'
                  ? 'There are no reported disputes on your purchase transactions.'
            ...
            disputes.map((d) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                  onPress={() => navigation.navigate('RecyclerDisputeDetail', { disputeId: d.id })}
                >
                  ...
                  <View style={styles.cardBody}>
                    <View style={styles.typeRow}>
                      ...
                      <Text style={styles.typeLabel}>{d.disputeType.replace(/_/g, ' ')}</Text>
                    </View>
                    ...
                    {d.resolvedWeightKg !== undefined && d.resolvedWeightKg !== null && (
                      <View style={styles.resolvedInfoBox}>
                        <Text style={styles.resolvedText}>
                          Resolved: <Text style={styles.bold}>{d.resolvedWeightKg} kg</Text>
                          {d.resolvedAmount ? ` • Amount: ₹${Number(d.resolvedAmount).toFixed(2)}` : ''}
                        </Text>
                      </View>
                    )}
                  </View>
                  ...
                  <View style={styles.cardFooter}>
                    <Text style={styles.viewDetailText}>Review & Respond →</Text>
                  </View>
                </TouchableOpacity>
              );
            })
